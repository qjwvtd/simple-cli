 ...
 * @param {*} ms,毫秒
 */
export function useEconomy(func:Function, ms:number) {
    let previous = 0;
 ...
 * @param {*} ms ,毫秒,默认3000
 */
export function useDebounce(func:Function, ms:number) {
    let timeout;
    return function(this:any) {
 ...
 **/
export const inputDebounce: { timerId: any; init: Function } = {
    timerId: null,
    init(fn:Function, delay:number) {
        return function(this:any) {
            if (inputDebounce.timerId) {
                clearTimeout(inputDebounce.timerId);
            }
            const args = { ...arguments };
            inputDebounce.timerId = setTimeout(() => {
                fn.apply(this, args);
            }, delay || 2000); //默认2秒
        };
    }
